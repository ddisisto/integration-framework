...
The model's own surprisal tracks this distinction directly. As @sec-information-surprisal established, surprisal and information content are the same quantity — Shannon's $-\log p(x)$. A token's surprisal is not a proxy for how much information it carries; it *is* how much information it carries, relative to the model's predictive distribution. High surprisal within structured context marks a token carrying information the context did not already contain — novel compressive work. Low surprisal marks a token the model could already predict — stabilising, not enriching. A token the model fully predicts from context carries, by Shannon's definition, no information the context did not already imply.
